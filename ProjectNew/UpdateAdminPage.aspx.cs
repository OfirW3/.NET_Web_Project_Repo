...
namespace ProjectNew
{
    ...
    {
        private DataSet dataSet = new DataSet(); // in memory collection of data
        ...
        public void UserStr()
        {
            ...
            using (SqlConnection connection = new SqlConnection(connectionString)) // אם הצליח לפתוח את הקשר למסד הנתונים תמשיך הלאה
            {
                ...
                SqlDataAdapter adapter = new SqlDataAdapter(sqlCommand); // Makes a "Bridge" with the database and dataset
                adapter.Fill(dataSet); // Fills the dataset with elements from the DB according to the query
            }
            ShowUserData();
        }
